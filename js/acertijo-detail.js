...
(function () {
  ...
  function render() {
    ...
    window.AcertijosStore.obtenerUno(id).then(function (acertijo) {
      if (!acertijo) {
      ...
      contenedor.innerHTML =
        ...
        (acertijo.resuelto ? plantillaResuelto(acertijo) : plantillaFormulario()) +
      ...
      if (!acertijo.resuelto) {
        document.getElementById('form-respuesta').addEventListener('submit', function (e) {
          ...
          boton.disabled = true;
          window.AcertijosStore.comprobarRespuesta(acertijo, input.value).then(function (correcto) {
